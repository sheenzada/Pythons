...
class CoffeeApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        title = ctk.CTkLabel(self, text="AI Coffee Assistant",
                             font=("Arial", 30, "bold"))
        title.pack(pady=20)

        self.name_entry = ctk.CTkEntry(self, placeholder_text="Customer Name")
        self.name_entry.pack(pady=10)

        self.mood_entry = ctk.CTkEntry(self, placeholder_text="Mood")
        self.mood_entry.pack(pady=10)

        self.weather_entry = ctk.CTkEntry(self, placeholder_text="Weather")
        self.weather_entry.pack(pady=10)

        ctk.CTkButton(self, text="Get AI Recommendation",
                      command=self.get_recommendation).pack(pady=20)

        self.output = ctk.CTkTextbox(self, width=700, height=250)
        self.output.pack(pady=20)

        ctk.CTkButton(self, text="Place Order",
                      command=self.place_order,
    ...
    def place_order(self):
        ...
        order = Order(customer, "AI Coffee", 9.99)

        self.orders.append(order)

        messagebox.showinfo("Order Placed", order.summary())