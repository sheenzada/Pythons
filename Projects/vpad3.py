...
align_left_btn = ttk.Button(tool_bar, text="⯇")
align_center_btn = ttk.Button(tool_bar, text="≡")
align_right_btn = ttk.Button(tool_bar, text="⯈")
...
def apply_alignment(justify):
    try:
        tag = justify
        text_editor.tag_configure(tag, justify=justify)
        text_editor.tag_add(tag, "sel.first", "sel.last")
    except tk.TclError:
        messagebox.showinfo("Align Text", "Please select text first.")
...
align_left_btn.config(command=lambda: apply_alignment(tk.LEFT))
align_center_btn.config(command=lambda: apply_alignment(tk.CENTER))
align_right_btn.config(command=lambda: apply_alignment(tk.RIGHT))
